...
class BirdEyeView:
    ...
    """
    def __init__(self, scale=100, size=(400, 400)):
        """
        Initialize Bird's Eye View
        
        Args:
            scale (float): Scale for depth (higher = more spread out)
    ...
    def reset(self):
        ...
        for y in range(0, self.height, grid_spacing):
            cv2.line(self.bev_image, (0, y), (self.width, y), grid_color, 1)
        
        # Draw vertical grid lines
        for x in range(0, self.width, grid_spacing):
            cv2.line(self.bev_image, (x, 0), (x, self.height), grid_color, 1)
        
        # Draw center line
        center_x = self.width // 2
        cv2.line(self.bev_image, (center_x, 0), (center_x, self.height), (0, 0, 100), 1)
